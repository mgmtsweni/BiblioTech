...
def employeelist():
    ...
    cursor.execute('SELECT *, oid FROM admindata')
    records = cursor.fetchall()
    ...
    def delete():
        cursor.execute(f"DELETE FROM admindata WHERE oid= {deleteBox.get()}")
        deleteBox.delete(0, END)
        connection.commit()
        connection.close()
    ...
    def updates():
        ...
        record_id = deleteBox.get()
        cursor.execute(f'SELECT * FROM admindata WHERE oid = {record_id}')
        records = cursor.fetchall()
        ...
        def edit():
            if passwordEntry.get() != confirmPassEntry.get():
                ...
                record_id = deleteBox.get()
                cursor.execute("""UPDATE admindata SET
                            name = :name,
                            email = :email,
                            username = :username,
                            password = :password
...
viewButton = Button(adminmainWin, text='Users', bd=0, cursor='hand2',
                    activebackground='tomato', activeforeground='white',
                    bg='white', fg="black", font=('Arial', 15, 'bold'), command=userelist)
viewButton.place(x=40, y=110)

viewButton = Button(adminmainWin, text='Admins', bd=0, cursor='hand2',
                    activebackground='tomato', activeforeground='white',
                    bg='white', fg="black", font=('Arial', 15, 'bold'), command=employeelist)
viewButton.place(x=140, y=110)
